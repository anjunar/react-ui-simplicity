...
import {AbstractTreeNode, ParagraphTreeNode, RootTreeNode} from "./core/TreeNode";
...
import {CursorHolder, CursorState} from "./core/Cursor";
import {SelectionHolder, SelectionState} from "./core/Selection";
import {EventState} from "./core/Event";
...
function Editor(properties: Editor.Attributes) {
    ...
    const [cursorState, setCursorState] = useState<CursorHolder>(() => {
        return {
            currentCursor: null
        }
    })

    const [selectionState, setSelectionState] = useState<SelectionHolder>(() => {
        return {
    ...
    let value = {
        ...
        cursor: {
            ...cursorState,
            triggerCursor(cursor?: CursorHolder) {
                if (cursor) {
    ...
    function onBlur() {
        ...
        console.log("blur remove cursor")
    }

    useEffect(() => {

        node.selected = !!(cursorState.currentCursor || selectionState.currentSelection);

        node.data.context = value

        trigger()

    }, [cursorState, selectionState]);
