import {AbstractContainerTreeNode, AbstractTreeNode, RootTreeNode, TextTreeNode} from "../core/TreeNode";
            ...
            if (parentIndex > -1) {
                ...
                if (siblingAbove instanceof AbstractContainerTreeNode) {
                    ...
                    let remainingOffset = parent
                        ...
                        .reduce((acc, child) => acc + (child as TextTreeNode).text.length, 0) + current.offset;

                    for (const child of siblingAbove.children) {
                        const textLength = (child as TextTreeNode).text.length;
        ...
        if (grandParent) {
            ...
            if (parentIndex >= 0 && parentIndex < grandParent.children.length - 1) {
                ...
                if (siblingBelow instanceof AbstractContainerTreeNode) {
                    ...
                    const lastTextNode = siblingBelow.children[siblingBelow.children.length - 1] as TextTreeNode;
                    if (lastTextNode) {
                        current.container = lastTextNode;
                        current.offset = lastTextNode.text.length;
                    }
