...
export function findNearestTextLeft(root: RootNode, parent: AbstractContainerNode<any>) : TextNode  {
    ...
    return flattened.find((node, index) => index > indexOf && (node instanceof TextNode || node instanceof CodeLineNode)) as TextNode
}
...
export function findNearestTextRight(root: RootNode, parent: AbstractContainerNode<any>) : TextNode {
    ...
    return flattened.findLast((node, index) => index < indexOf && (node instanceof TextNode || node instanceof CodeLineNode)) as TextNode
}
...
export function onArrowLeft(root: RootNode, current: { container: AbstractNode; offset: number }) {
    ...
    if (indexOf > 0) {
        let container = flattened.findLast((node, index) => index < indexOf && (node instanceof TextNode || node instanceof CodeLineNode));
        if (container) {
...
export function onArrowRight(root: RootNode, current: { container: AbstractNode; offset: number }) {
    ...
    if (indexOf < flattened.length - 1) {
        current.container = flattened.find((node, index) => index > indexOf && (node instanceof TextNode || node instanceof CodeLineNode))
        current.offset = 0
    }
}

export function onArrowUp(node: TextNode | CodeLineNode, current: { container: AbstractNode; offset: number }, root: RootNode) {
...
export function onArrowDown(node: TextNode | CodeLineNode, current: { container: AbstractNode; offset: number }, root : RootNode) {
    let parent = node.parent;
    if (parent) {
        let grandParent = parent.parent;
        if (grandParent) {
